...
[CustomEntity("KoseiHelper/DashCounter")]
[Tracked]
public class DashCounter : Component
{
    ...
    private MTexture dashIcon, demoIcon;
    ...
    public float FullHeight => Math.Max(ActiveFont.LineHeight, dashIcon.Height);
    public Vector2 RenderPosition => (((base.Entity is not null) ? base.Entity.Position : Vector2.Zero) + Position).Round();

    public DashCounter(bool centeredX, int amount, int outOf = 0, bool showOutOf = false, MTexture texture = null)
        : base(active: true, visible: true)
    {
        ...
        dashIcon = texture ?? GFX.Gui["KoseiHelper/dashcount/dash"];
        demoIcon = texture ?? GFX.Gui["KoseiHelper/dashcount/demo"];
        xIcon = GFX.Gui["x"];
    }
    ...
    public override void Render()
    {
        ...
        string dashCountString = sAmount;
        float textlength = ActiveFont.Measure(dashCountString).X;
        ...
        // Draw a dash icon
        //*The dashIcon will render blurry on resolutions lower than 6x pixel scale (Lower than 1080p). 
        // There is no fix to this as far as I know.
        dashIcon.DrawCentered(Calc.Floor(renderPosition + new Vector2(1f, 0f) * 60f * 0.5f), Color.White, 1f);
